...
def _whole_share_target(position_shares: float) -> float:
    return float(math.trunc(position_shares))
...
async def run_live(
    ...
    requested_symbol = str(symbol).strip() if symbol is not None else ""
    execution_symbol = requested_symbol or settings.ib_symbol
    if not execution_symbol:
        raise ValueError("A live execution symbol is required.")

    execution_contract_id = settings.ib_contract_id
    data_settings = settings
    if execution_symbol != settings.ib_symbol:
        execution_contract_id = 0
        data_settings = settings.model_copy(
            update={"ib_symbol": execution_symbol, "ib_contract_id": 0}
        )

    bars = await fetch_ibkr_bars_async(data_settings)
    if not bars:
    ...
    try:
        ...
        if not dry_run:
            ...
            if not managed_accounts:
                raise RuntimeError(
                    "Unable to verify execution.account because IBKR returned no managed accounts."
                )
        ...
        if signal.target_position == current_unit:
            ...
            if (
                ...
                target_position = candidate_target
        target_position = _whole_share_target(target_position)
        delta = target_position - current_position
        if 0 < abs(delta) < 1.0:
            target_position = current_position
            delta = 0.0

        order_id = "dry-run" if dry_run else "none"
