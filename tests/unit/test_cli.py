...
class _EmptyManagedAccountsBroker(_FakeBroker):
    def get_managed_accounts(self) -> list[str]:
        return []
...
def test_live_rejects_symbol_that_differs_from_data_config(tmp_path: Path) -> None:
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
...
def test_live_uses_prior_bar_for_signal(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    async def fake_fetch_bars(settings: object) -> list[Bar]:
        del settings
        start = datetime(2025, 1, 1)
    ...
    runner.TWS_Wrapper_Client = _FakeBroker
    monkeypatch.setattr(runner, "fetch_ibkr_bars_async", fake_fetch_bars)
    config_path = Path(_write_isolated_config(tmp_path, "configs/paper.yaml"))
    cfg = yaml.safe_load(config_path.read_text())
    cfg["strategy"]["lookback"] = 1
    cfg["strategy"]["entry_threshold"] = 0.05
    cfg["risk"]["target_notional"] = 100
    config_path.write_text(yaml.safe_dump(cfg, sort_keys=False))

    result = asyncio.run(runner.run_live(config_path=str(config_path), dry_run=True))
...
def test_live_non_dry_run_requires_managed_accounts(tmp_path: Path) -> None:
    runner.TWS_Wrapper_Client = _EmptyManagedAccountsBroker
    data_pipeline.TWS_Wrapper_Client = _EmptyManagedAccountsBroker  # type: ignore[assignment]
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
    with pytest.raises(RuntimeError, match="Unable to confirm managed IBKR accounts"):
        asyncio.run(runner.run_live(config_path=config_path, dry_run=False, symbol="AAPL"))
