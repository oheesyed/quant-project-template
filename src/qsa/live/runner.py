...
async def run_live(
    ...
    live_symbol = configured_symbol if symbol is None else symbol.strip()
    if not live_symbol:
        raise ValueError("Live symbol cannot be empty.")

    data_settings = settings
    data_updates: dict[str, str | int] = {}
    if live_symbol != settings.ib_symbol:
        data_updates["ib_symbol"] = live_symbol
    if live_symbol != configured_symbol:
        data_updates["ib_contract_id"] = 0
    if data_updates:
        data_settings = settings.model_copy(update=data_updates)
