...
def _settings_for_live_symbol(
    settings: Settings, symbol: str | None
) -> tuple[Settings, str]:
    live_symbol = settings.ib_symbol if symbol is None else symbol.strip()
    if not live_symbol:
        raise ValueError("Live trading symbol cannot be empty.")
    if live_symbol == settings.ib_symbol:
        return settings, live_symbol

    # A config-level contract id belongs to its configured symbol; do not reuse
    # it for an ad-hoc CLI symbol override.
    return settings.model_copy(
        update={"ib_symbol": live_symbol, "ib_contract_id": 0}
    ), live_symbol
...
async def run_live(
    ...
    settings, live_symbol = _settings_for_live_symbol(settings, symbol)
