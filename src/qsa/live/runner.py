...
def _settings_for_live_symbol(settings: Settings, symbol: str | None) -> tuple[Settings, str]:
    ...
    live_symbol = (symbol if symbol is not None else configured_symbol).strip()
    if not live_symbol:
        raise ValueError("Live symbol must be provided by --symbol or data.ib_symbol.")
    if live_symbol == configured_symbol:
        return settings, live_symbol
    return (
        settings.model_copy(update={"ib_symbol": live_symbol, "ib_contract_id": 0}),
        live_symbol,
    )
...
async def run_live(
    ...
    data_settings, live_symbol = _settings_for_live_symbol(settings, symbol)
    ...
    try:
        ...
        if not dry_run and delta != 0:
            order_id = await broker.place_market_order(
                ...
                contract_id=data_settings.ib_contract_id,
                exchange=data_settings.ib_exchange,
            )
