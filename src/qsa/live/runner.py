...
def _whole_share_target(position_shares: float) -> float:
    if position_shares > 0:
        return float(math.floor(position_shares))
    if position_shares < 0:
        return float(math.ceil(position_shares))
    return 0.0


def _whole_share_delta(delta: float) -> float:
    if delta > 0:
        return float(math.floor(delta))
    if delta < 0:
        return float(math.ceil(delta))
    return 0.0
...
async def run_live(
    ...
    execution_symbol = configured_symbol if symbol is None else symbol.strip()
    if not execution_symbol:
        raise ValueError("A non-empty live execution symbol is required.")
    if execution_symbol != configured_symbol:
        raise ValueError(
            f"Live execution symbol '{execution_symbol}' must match configured "
            f"data.ib_symbol '{configured_symbol}'. Update the config to trade a different symbol."
        )
    ...
    try:
        ...
        if not dry_run:
            ...
            if not managed_accounts:
                raise RuntimeError(
                    "Unable to validate execution.account: no managed accounts returned."
                )
        ...
        if signal.target_position == current_unit:
            ...
            if (
                ...
                target_position = candidate_target
        delta = _whole_share_delta(target_position - current_position)
