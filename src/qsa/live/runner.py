...
async def run_live(
    ...
    requested_symbol = None if symbol is None else str(symbol).strip()
    resolved_symbol = requested_symbol or settings.ib_symbol.strip()
    if not resolved_symbol:
        raise ValueError("A live trading symbol is required.")
    if requested_symbol and requested_symbol != settings.ib_symbol:
        settings = settings.model_copy(
            update={"ib_symbol": requested_symbol, "ib_contract_id": 0}
        )
