...
async def run_live(
    ...
    effective_symbol = (symbol or settings.ib_symbol).strip()
    if not effective_symbol:
        raise ValueError("A live execution symbol is required.")
    data_settings = settings
    if effective_symbol != settings.ib_symbol:
        data_settings = settings.model_copy(
            update={"ib_symbol": effective_symbol, "ib_contract_id": 0}
        )
    ...
    try:
        ...
        current_position = broker.get_position(effective_symbol)
        ...
        if not dry_run and delta != 0:
            order_id = await broker.place_market_order(
                symbol=effective_symbol, quantity=delta, price_hint=last_price
            )
