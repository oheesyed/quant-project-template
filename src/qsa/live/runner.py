...
def _resolve_live_symbol(configured_symbol: str, requested_symbol: str | None) -> str:
    ...
    if not requested:
        raise ValueError("--symbol cannot be empty for live execution.")
    if requested.casefold() != configured.casefold():
        raise ValueError(
            f"Live symbol mismatch: --symbol '{requested}' does not match "
            f"data.ib_symbol '{configured}'. Update the config instrument or omit --symbol."
        )
    return configured
...
async def run_live(
    ...
    execution_symbol = _resolve_live_symbol(settings.ib_symbol, symbol)
