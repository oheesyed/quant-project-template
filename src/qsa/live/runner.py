...
async def run_live(
    ...
    if not resolved_symbol:
        raise ValueError("Live execution requires a symbol via --symbol or data.ib_symbol.")

    data_settings = settings
    if resolved_symbol != settings.ib_symbol:
        data_settings = settings.model_copy(
            update={"ib_symbol": resolved_symbol, "ib_contract_id": 0}
        )
