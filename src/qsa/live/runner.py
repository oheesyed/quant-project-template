...
async def run_live(
    ...
    requested_symbol = settings.ib_symbol if symbol is None else symbol
    execution_symbol = requested_symbol.strip()
    if not execution_symbol:
        raise ValueError("Live symbol must be non-empty.")
