...
async def run_live(
    ...
    trade_symbol = (symbol or configured_symbol).strip()
    if not trade_symbol:
        raise ValueError("Live execution symbol must be configured or provided.")
    contract_id = settings.ib_contract_id if trade_symbol == configured_symbol else 0
    data_settings = settings.model_copy(
        update={"ib_symbol": trade_symbol, "ib_contract_id": contract_id}
    )
