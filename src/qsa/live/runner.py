...
async def run_live(
    ...
    if symbol is not None and symbol != settings.ib_symbol:
        raise ValueError(
            f"Live symbol override '{symbol}' does not match configured data.ib_symbol "
            f"'{settings.ib_symbol}'. Update the config so historical data, positions, "
            "and orders use the same instrument."
        )
    execution_symbol = settings.ib_symbol
