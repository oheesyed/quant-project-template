...
def _settings_for_live_symbol(settings: Settings, symbol: str | None) -> tuple[Settings, str]:
    execution_symbol = (symbol or settings.ib_symbol).strip()
    if not execution_symbol:
        raise ValueError("Live execution symbol must be provided by config or --symbol.")
    if execution_symbol == settings.ib_symbol:
        return settings, execution_symbol
    return settings.model_copy(
        update={
            "ib_symbol": execution_symbol,
            "ib_contract_id": 0,
        }
    ), execution_symbol


def _whole_share_position(position_shares: float) -> float:
...
async def run_live(
    ...
    settings, execution_symbol = _settings_for_live_symbol(load_settings(config_path), symbol)
    ...
    try:
        ...
        signal_bars = bars[:-1]
        signal = strategy.generate_signal(signal_bars, current_position=current_unit)
        ...
        if signal.target_position == current_unit:
            ...
            candidate_target = _whole_share_position(
                clamp_target_position(raw_target, settings.max_abs_position)
            )
            is_entry_or_flip = (
