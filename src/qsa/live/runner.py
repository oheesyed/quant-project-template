...
def _resolve_execution_symbol(configured_symbol: str, requested_symbol: str | None) -> str:
    execution_symbol = str(configured_symbol).strip()
    if not execution_symbol:
        raise ValueError("data.ib_symbol is required for live execution.")

    requested = "" if requested_symbol is None else str(requested_symbol).strip()
    if requested and requested.upper() != execution_symbol.upper():
        raise ValueError(
...
async def run_live(
    ...
    try:
        ...
        delta = _whole_share_delta(desired_delta)
