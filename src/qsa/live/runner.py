...
def _resolve_live_symbol(configured_symbol: str, requested_symbol: str | None) -> str:
    data_symbol = configured_symbol.strip()
    if not data_symbol:
    ...
    live_symbol = requested_symbol.strip()
    if live_symbol != data_symbol:
        raise ValueError(
            f"Live execution symbol '{live_symbol}' must match data.ib_symbol "
            f"'{data_symbol}' so signals and orders use the same instrument."
        )
    return live_symbol
    ...
    try:
        ...
        if not dry_run:
            ...
            if not managed_accounts:
                raise RuntimeError(
                    f"Unable to verify configured execution.account '{configured_account}' "
                    "because IBKR returned no managed accounts."
                )
