...
def test_live_rejects_mismatched_symbol_before_fetch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    async def fail_fetch(*args: object, **kwargs: object) -> list[object]:
        del args, kwargs
        raise AssertionError("live runner should reject mismatched symbols before fetching data")

    monkeypatch.setattr(runner, "fetch_ibkr_bars_async", fail_fetch)
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
    with pytest.raises(ValueError, match="Live symbol mismatch"):
