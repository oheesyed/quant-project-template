...
def test_live_whole_share_policy_skips_sub_share_target(tmp_path: Path) -> None:
    _RecordingBroker.reset()
    runner.TWS_Wrapper_Client = _RecordingBroker
    data_pipeline.TWS_Wrapper_Client = _RecordingBroker  # type: ignore[assignment]
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
...
def test_live_non_dry_run_rejects_empty_managed_accounts(tmp_path: Path) -> None:
    ...
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
    with pytest.raises(RuntimeError, match="no managed accounts returned"):
        asyncio.run(runner.run_live(config_path=config_path, dry_run=False, symbol="AAPL"))
...
def test_tws_market_order_rejects_fractional_quantity_before_submission() -> None:
