...
class _RejectingBroker(_FakeBroker):
    async def place_market_order(
        ...
        raise RuntimeError(
            "IBKR rejected market order 4 for TEST: status=ValidationError."
        )
...
def test_live_rejects_symbol_that_does_not_match_config(tmp_path: Path) -> None:
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
...
def test_live_non_dry_run_skips_sub_share_order(tmp_path: Path) -> None:
    _FakeBroker.placed_orders.clear()
    runner.TWS_Wrapper_Client = _FakeBroker
    data_pipeline.TWS_Wrapper_Client = _FakeBroker  # type: ignore[assignment]
    config_path = _write_isolated_config(tmp_path, "configs/paper.yaml")
...
def test_live_non_dry_run_uses_configured_contract(tmp_path: Path) -> None:
    ...
    config_path = _write_isolated_config(
        tmp_path, "configs/paper.yaml", target_notional=1_000.0
    )
    result = asyncio.run(runner.run_live(config_path=config_path, dry_run=False, symbol="AAPL"))
